...
- [previous_chapters.py](previous_chapters.py) 是一个 Python 模块，包含我们在前几章中编码和训练的 GPT 模型，以及许多实用程序函数，我们在本章中重用它们
...
- [gpt_instruction_finetuning.py](gpt_instruction_finetuning.py) 是一个独立的 Python 脚本，用于按照主要章节中的描述对模型进行指令微调（可以将其视为专注于微调部分的章节摘要）
...
- [ollama_evaluate.py](ollama_evaluate.py) 是一个独立的 Python 脚本，用于评估微调模型的响应，如主要章节所述（可以将其视为专注于评估部分的章节摘要）
...
- [exercise_experiments.py](exercise_experiments.py) 是一个可选脚本，用于实现练习解答；有关更多详细信息，请参阅 [exercise-solutions.ipynb](exercise-solutions.ipynb)
